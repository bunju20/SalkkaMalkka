...
const Options = () => {
    ...
    const [mbtiCounts, setMbtiCounts] = useState({I: 0, E: 0, S: 0, N: 0, T: 0, F: 0, J: 0, P: 0});
    const [finalMbti, setFinalMbti] = useState("");

    const updateMbtiCountAndNavigate = (type) => {
        setMbtiCounts(prevCounts => {
            const updatedCounts = { ...prevCounts, [type]: prevCounts[type] + 1 };
    ...
    const calculateMbtiResult = (counts) => {
        ...
        return mbtiPairs.map(pair => {
            ...
            return counts[firstType] >= counts[secondType] ? firstType : secondType;
        }).join('');
    };
                            ...
                            <div className={styles.content} key={item.id}>
                                ...
                                <div className={styles.top}>
                                    <div className={styles.mbti__counter}>
                                        <span className={styles.mbti__progress__color}>{currentSlide}</span>
                                        <span className={styles.mbti__end__color}>/{TOTAL_SLIDES}</span>
                                    </div>
                                    <h1 className={styles.mbti__question}>{item.question}</h1>
                                </div>
                                <article className={styles.mbti__btn__box}>
                                    {item.answers.map((answer, answerIndex) => (
                                        <button key={answer.type} className={styles.mbti__button} onClick={() => nextSlide(answerIndex)}>
                                            {answer.content}
                                        </button>
                                    ))}
                                </article>
